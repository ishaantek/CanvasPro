...
const applyVarsURL = chrome.runtime.getURL(
...
const applyAppearance = (appearance) => {
  if (appearance && appearance !== "light") {
    ...
    varSheet.href = chrome.runtime.getURL(
      "src/inject/colors/" + appearance + ".css"
    );
  } else {
    linkApplyVars.href = "";
    varSheet.href = "";
...
window
  ...
  .addEventListener("change", (e) => {
    if (originalappearance.endsWith("auto"))
      applyAppearance(
        (appearance = window.matchMedia("(prefers-color-scheme: dark)").matches
          ? originalappearance.substring(0, originalappearance.length - 5)
          : "light")
      );
  });
...
useReactiveFeature("canvaspro-display-appearance", (color) => {
  appearance = originalappearance = color;
  originalappearance.endsWith("auto")
    ? (appearance = window.matchMedia("(prefers-color-scheme: dark)").matches
        ? originalappearance.substring(0, originalappearance.length - 5)
        : "light")
    : color;
